  ...
  return (
    ...
    <div className='w-full min-h-20 bg-violet-900 p-2 text-white md:flex items-center fixed'>
     ...
     {show && <div className='mt-4 flex md:hidden'>
        <Link to={'/wishlist'}><button className='flex otems-center p-4 border border-white rounded me-4 hover:bg-white hover:text-violet-900'><FontAwesomeIcon icon={faHeart} className='me-2' style={{color:'red'}} />Wishlist <span className='px-2 border-slate-500 bg-slate-400 rounded ms-2'>{wishlistArray?.length}</span></button></Link>
      ...
      <div className='md:flex ms-auto hidden'>
        <Link to={'/wishlist'}><button className='flex otems-center p-4 border border-white rounded me-4 hover:bg-white hover:text-violet-900'><FontAwesomeIcon icon={faHeart} className='me-2' style={{color:'red'}} />Wishlist <span className='px-2 border-slate-500 bg-slate-400 rounded ms-2'>{wishlistArray?.length}</span></button></Link>
        <Link to={'/cart'}><button className='flex otems-center p-4 border border-white rounded me-4 hover:bg-white hover:text-violet-900'><FontAwesomeIcon icon={faCartShopping} className='me-2' style={{color:'green'}} />Cart <span className='px-2 border-slate-500 bg-slate-400 rounded ms-2'>{cartArray?.length}</span></button></Link>
      </div>
    </div>
    </>
  )
